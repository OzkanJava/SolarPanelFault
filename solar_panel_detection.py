...
def main():
    ...
    # 13) Karışıklık Matrisi ve Korelasyon Analizi
    print("\nKarışıklık Matrisi ve Korelasyon Analizi Yapılıyor...")

    # Tüm etiket ve tahminleri toplamak için liste
    all_labels = []
    all_predictions = []

    # Test verilerini kullanarak tüm tahminleri toplama
    model.eval()
    with torch.no_grad():
        for images, labels in test_loader:
            images = images.to(device)
    ...
    cm_correlation = np.corrcoef(cm_normalized)

    plt.figure(figsize=(8, 6))
    sns.heatmap(cm_correlation, annot=True, fmt=".2f", cmap='coolwarm', 
                xticklabels=class_names, yticklabels=class_names)
    plt.title("Sınıflar Arası Tahmin Korelasyonu")
    plt.xlabel("Tahmin Edilen Sınıflar")
    plt.ylabel("Gerçek Sınıflar")
    plt.show()

    print("Karışıklık Matrisi ve Korelasyon Analizi Tamamlandı.")
